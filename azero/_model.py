...
class Losses(collections.namedtuple("Losses", "policy value l2")):
    ...
    def __str__(self):
        return (
            f"Losses(total: {self.total:.3f}, "
            f"policy: {self.policy:.3f}, "
...
class Model:
    ...
    @classmethod
    def build_model(
        cls, model_type, input_shape, output_size,
        nn_width, nn_depth, weight_decay, learning_rate, path, *,
        is_win_loose=True
    ):
    ...
    @staticmethod
    def _define_graph(
        model_type, input_shape, output_size,
        ...
        _ = tf.identity(tfkl.Softmax()(policy_logits), name="policy_softmax")
        policy_targets = tf.placeholder(shape=[None, output_size], dtype=tf.float32, name="policy_targets")
        policy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=policy_logits, labels=policy_targets), name="policy_loss")
        ...
        if is_win_loose:
            _val_out_layers.append(tfkl.Activation("tanh"))
        value_out = cascade(value_head, _val_out_layers)
        ...
        with tf.control_dependencies(bn_updates):
            _ = optimizer.minimize(total_loss, name="train")
    ...
    def write_graph(self, filename, as_text=False):
    ...
    def inference(self, observation, legals_mask):
        fetches = [self._value_out, self._policy_softmax]
        feed_dict = {
    ...
    def update(self, train_inputs: Sequence[TrainInput]):
        ...
        _, p, v, l2 = self._session.run(fetches, feed_dict=feed_dict)
        return Losses(p, v, l2)

    def save_checkpoint(self, step):
        _path = os.path.join(self._path, "checkpoint")
        return self._saver.save(self._session, _path, global_step=step)
